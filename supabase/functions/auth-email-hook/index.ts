...
import {
  ...
  type TemplateData,
} from '../_shared/email-templates/templates.ts'
...
// Template mapping — each returns { html, text }
const EMAIL_TEMPLATES: Record<string, (data: TemplateData) => { html: string; text: string }> = {
  signup: renderSignup,
  invite: renderInvite,
  magiclink: renderMagicLink,
  recovery: renderRecovery,
  email_change: renderEmailChange,
  reauthentication: renderReauthentication,
}
...
const SAMPLE_PROJECT_URL = "https://cozy-starter-kit-83.lovable.app"
...
async function handlePreview(req: Request): Promise<Response> {
  ...
  const renderFn = EMAIL_TEMPLATES[type]
  if (!renderFn) {
  ...
  const { html } = renderFn(sampleData)
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  const renderFn = EMAIL_TEMPLATES[emailType]
  if (!renderFn) {
  ...
  const { html, text } = renderFn(templateData)
