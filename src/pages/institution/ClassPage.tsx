...
import { Download, FileText, Image as ImageIcon, Link as LinkIcon, Lock, MessageCircle, Plus, UsersRound } from "lucide-react";
...
import { classDocs, makers, feed, professorClasses } from "@/lib/fixtures";
...
/** 32 · Generic private class page (professor-side) — documents, shared pins, group chat. */
    ...
    <MobileShell>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[22px] pb-4 pt-3">
        ...
        <div className="mb-3.5 flex items-center gap-3">
          ...
          >
            <Plus size={19} className="text-tg-emph-text" />
          </button>
        </div>
        ...
        {tab === "Documents" && (
          <>
            <div className="mb-2.5 mt-[18px] flex items-baseline justify-between">
              <span className="font-display text-[13px] font-semibold text-tg-ink">Documents</span>
              <button
                type="button"
                onClick={() => navigate(routes.professorUploadDoc)}
            ...
            {students.map((m) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Meta className="mt-0.5 block">{cls.tag}</Meta>
                </div>
              </button>
            ))}
          </div>
